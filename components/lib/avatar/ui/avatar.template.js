import { COLOR, SHAPE, WCATTR } from '../../../consts/index.js';
import { html } from '../../../utils/index.js';

export const template = html`
  <template>
    <style>
      :host {
        display: contents;
      }

      ::slotted(img),
      ::slotted(picture) {
        width: var(--${WCATTR.SizeNumber});
        height: var(--${WCATTR.SizeNumber});
        object-fit: cover;
        object-position: center;
        position: absolute;
        inset: 0;
      }

      .wrapper {
        display: grid;
        width: var(--${WCATTR.SizeNumber});
        height: var(--${WCATTR.SizeNumber});
        font-size: var(--${WCATTR.SizeNumber});

        .text {
          font-size: 0.35em;
          line-height: 1;
          user-select: none;
          color: var(--text-avatar-color);
        }
      }

      .bg {
        position: relative;
        display: grid;
        place-items: center;
      }

      :host([${WCATTR.Color}='${COLOR.Red}']) .bg {
        background-image: var(--gradient-red);
      }

      :host([${WCATTR.Color}='${COLOR.Green}']) .bg {
        background-image: var(--gradient-green);
      }

      :host([${WCATTR.Color}='${COLOR.Orange}']) .bg {
        background-image: var(--gradient-orange);
      }

      :host([${WCATTR.Color}='${COLOR.Blue}']) .bg {
        background-image: var(--gradient-blue);
      }

      :host([${WCATTR.Color}='${COLOR.Purple}']) .bg {
        background-image: var(--gradient-purple);
      }

      :host([${WCATTR.Shape}='${SHAPE.Squircle}']) .bg {
        mask-image: url(data:image/svg+xml;base64,);
        mask-position: center;
        mask-repeat: no-repeat;
        mask-size: contain;
        mask-type: alpha;
      }

      :host([${WCATTR.Shape}='${SHAPE.Circle}']) .bg {
        mask-image: url(data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI1NiIgaGVpZ2h0PSI1NiIgZmlsbD0ibm9uZSI+PGNpcmNsZSBjeD0iMjgiIGN5PSIyOCIgcj0iMjgiIGZpbGw9IiMwMDAiLz48L3N2Zz4=);
        mask-position: center;
        mask-repeat: no-repeat;
        mask-size: contain;
        mask-type: alpha;
      }
    </style>

    <span class="wrapper">
      <span class="bg">
        <span class="text">
          <slot><wc-icon name="user-round"></wc-icon></slot>
        </span>
      </span>
    </span>
  </template>
`;
